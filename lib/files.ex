defmodule Huffman.Files do
  ...
  defp serialize(mapping) do
    ...
    |> Enum.reduce(initial_text, fn({width, mappings}, acc) ->
      symbols = Enum.sort(Map.values(mappings))
  ...
  defp deserialize(<<48, 32, remaining::bitstring>>, _to_read_width, _group_width, m),
    do: {remaining, Huffman.Decoder.prep_map_for_decoding(Huffman.Encoder.assign_ascending_bits(Enum.reverse(m), 0))}

  defp deserialize(header, to_read_width, group_width, m) do
    <<first::binary-size(to_read_width), second::binary-size(group_width), remaining_header::bitstring>> = header
    number_of_bytes_to_read = String.to_integer(first)
    bit_length = String.to_integer(second)
    <<bytes_for_group::binary-size(number_of_bytes_to_read), remaining::bitstring>> = remaining_header
    ...
    deserialize(remaining, to_read_width, group_width, [group_mapping | m])
  end
  ...
  defp write_file(uncompressed_text, path) do
    case File.write(path, uncompressed_text) do
      ...
      {:error, reason} -> "Unable to decompress data: #{path} #{reason}"
    end
  end
  ...
  def decompress(path, out_file_path \\ nil) do
    case File.read(path) do
      {:ok, <<number_of_leftover_bits::binary-size(1), first::binary-size(1), second::binary-size(1), compressed_data::binary>>} ->
        ...
        <<compressed_data_without_extra_bits::size(bit_diff), _junk_bits::bitstring>> = compressed_data

        {encoded_data, mapping} = deserialize(
          <<compressed_data_without_extra_bits::size(bit_diff)>>,
          bytes_to_read_width,
          group_size_width,
          []
        )
      ...
      {:error, reason} -> "Unable to read input file: #{reason}"
    end
  end
end
